...
function logPetTransportQueryFallback(error: unknown, scope: string) {
  if (process.env.APP_ENV !== "development") {
    return;
  }

  console.warn(`Unable to load ${scope}`, {
    message: error instanceof Error ? error.message : "Unknown pet transport query error",
...
export async function getPetTransportsForUser(
  ...
  try {
    ...
    logPetTransportQueryFallback(error, "pet shipments");
...
export async function getPetTransportForUser(
  ...
  try {
    ...
    logPetTransportQueryFallback(error, "pet shipment details");
